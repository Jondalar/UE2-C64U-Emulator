...
| TRX64 is a git dependency pinned by rev | `crates/c64-bridge/Cargo.toml` names the rev (TRX64 `1ce84b0`, with Spec 855's several SIDs). Cargo fetches it on the first build. The bridge drives TRX64 internals, so the tests and the C64 smokes run before `rev` moves. A local checkout is patched in through an untracked `.cargo/config.toml` | [install.md](status/install.md) "TRX64 dependency" |
