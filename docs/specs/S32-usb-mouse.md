...
**Status:** the USB side is built (2026-09-24): the firmware finds the mouse, moves port 1's paddle values and
presses its fire line. The C64 reading POTX/POTY waits for TRX64 Spec 876 (POT values per control port).
...
- `crates/ue2emu/src/usb.rs` (`--usb-mouse`), `window.rs` (capture), `control.rs` (`usbmouse`), `monitor/devices.rs`
  (`joy`)
- next: `crates/c64-bridge` (core registers 0x13, 0x1A, 0x32-0x37 → `Machine::set_pot`)
...
latched every 512 cycles, $FF when nothing is set. The bridge will latch PADDLE_n_X/Y and hand them over.
...
- Once Spec 876 is in: GEOS with a 1351 driver follows the mouse.
