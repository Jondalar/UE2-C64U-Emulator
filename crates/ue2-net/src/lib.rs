...
//!
//! - [`UserNet`] is libslirp user-mode networking: a virtual 10.0.2.0/24 segment where the host is 10.0.2.2, DNS
//!   is 10.0.2.3 and DHCP hands the guest 10.0.2.15, plus port forwards from the Mac into the guest. libslirp is not
//!   thread-safe.
//! - [`socket_vmnet::SocketVmnet`] connects to lima's socket_vmnet daemon, which owns a vmnet interface as root; the
//!   guest's DHCP goes to the daemon's network (the LAN when the daemon runs bridged).
...
use libc::{in6_addr, in_addr, pollfd};
...
fn in_addr_of(ip: Ipv4Addr) -> in_addr {
    in_addr { s_addr: u32::from_ne_bytes(ip.octets()) }
}

const IN6_ANY: in6_addr = in6_addr { s6_addr: [0; 16] };
...
const POLL_FLAGS: [(c_int, libc::c_short); 5] = [
...
unsafe extern "C" fn add_poll(fd: c_int, events: c_int, opaque: *mut c_void) -> c_int {
    let fds = &mut *opaque.cast::<Vec<pollfd>>();
    let events = POLL_FLAGS.iter().filter(|(s, _)| events & s != 0).fold(0, |acc, (_, p)| acc | p);
    fds.push(pollfd { fd, events, revents: 0 });
    fds.len() as c_int - 1
}

unsafe extern "C" fn get_revents(idx: c_int, opaque: *mut c_void) -> c_int {
    let fds = &*opaque.cast::<Vec<pollfd>>();
    let Some(fd) = usize::try_from(idx).ok().and_then(|i| fds.get(i)) else { return 0 };
    POLL_FLAGS.iter().filter(|(_, p)| fd.revents & p != 0).fold(0, |acc, (s, _)| acc | s)
}
